...
def recommend_single_option(options_data, S, probability_ITM):
    ...
    otm_options['probability_ITM'] = calculate_probability_ITM(S, otm_options['strike'], 14 / 365, RISK_FREE_RATE, 0.2)
...
def main():
    ...
    # Clear the sheet and add headers once at the start
    sheet.clear()
    header = ["Ticker", "Current Price", "Recommended Option Type", "Recommended Option",
              "Strike", "Premium", "Expiry", "Market Edge"]
    ...
    while True:
        ...
        for count, ticker in enumerate(tickers, start=1):
            ...
            if stock_info:
                # Update Google Sheets immediately after analyzing each stock
                sheet.append_row(list(stock_info.values()))
            
            # Pause for 10 seconds after analyzing each stock
            #time.sleep(10)
        
        print("\nCompleted analysis cycle.")
        
        print("Waiting 60 minutes before the next check...")
        time.sleep(3600)
